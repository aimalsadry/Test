...
// Fix for JSX intrinsic element type errors (Three.js elements) in certain TS environments
...
const CameraController = ({ z, fov }: { z: number; fov: number }) => {
...
const MacroscopicWave = ({ scale = 1 }: { scale?: number }) => {
  const ref = useRef<THREE.Mesh>(null);
  ...
  return (
    <Torus ref={ref} args={[3 * scale, 0.1, 16, 100]} rotation={[Math.PI / 2, 0, 0]}>
  ...
  return (
    <div className="absolute inset-0 z-0 opacity-60 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 6], fov: 45 }}>
        <CameraController z={isMobile ? 8 : 6} fov={isMobile ? 50 : 45} />
        <AmbientLight intensity={0.5} />
        <PointLight position={[10, 10, 10]} intensity={1} />
        <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.5}>
          <QuantumParticle position={[0, 0, 0]} color="#4F46E5" scale={1.2 * s} />
          <MacroscopicWave scale={s} />
        </Float>
        
        <Float speed={2} rotationIntensity={0.5} floatIntensity={1}>
           <QuantumParticle position={[-3 * s, 1 * s, -2]} color="#9333EA" scale={0.5 * s} />
           <QuantumParticle position={[3 * s, -1 * s, -3]} color="#C5A059" scale={0.6 * s} />
        </Float>
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      <Canvas camera={{ position: [0, 0, 4.5], fov: 45 }}>
        <CameraController z={isMobile ? 6.5 : 4.5} fov={isMobile ? 50 : 45} />
        ...
        <Float rotationIntensity={0.4} floatIntensity={0.2} speed={1}>
          <Group rotation={[0, 0, 0]} position={[0, 0.5 * s, 0]} scale={[s, s, s]}>
